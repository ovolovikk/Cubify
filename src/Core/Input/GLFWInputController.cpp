...
static GLFWInputController *s_currentController = nullptr;

static void scrollCallback(GLFWwindow *window, double xoffset, double yoffset) {
  if (s_currentController) {
    s_currentController->accumulateScroll(static_cast<float>(yoffset));
  }
}

GLFWInputController::GLFWInputController(GLFWwindow *window_)
    : window(window_), last_x(0), last_y(0), delta_x(0), delta_y(0),
      scroll_delta(0), first_mouse(true) {
  if (window) {
    int w, h;
    ...
    s_currentController = this;
    glfwSetScrollCallback(window, scrollCallback);
  }
}

void GLFWInputController::accumulateScroll(float y) { scroll_delta += y; }
...
float GLFWInputController::getScrollDelta() {
  float delta = scroll_delta;
  scroll_delta = 0;
  return delta;
}
...
bool GLFWInputController::isCursorEnabled() const { return cursor_enabled; }
...
bool GLFWInputController::wasMouseButtonJustPressed(int button) const {
  ...
  if (mutableThis.m_currMouseState.find(button) ==
      mutableThis.m_currMouseState.end()) {
  ...
  bool currPressed =
      (currIt != m_currMouseState.end()) ? currIt->second : false;
  bool prevPressed =
      (prevIt != m_prevMouseState.end()) ? prevIt->second : false;
...
bool GLFWInputController::isKeyPressed(int key) const {
  auto &mutableThis = const_cast<GLFWInputController &>(*this);
  if (mutableThis.m_currKeyState.find(key) ==
      mutableThis.m_currKeyState.end()) {
